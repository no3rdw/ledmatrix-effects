import time, displayio, vectorio, os, bitmaptools
...
class Effect(Effect):
	...
	def handleRemote(self, key:str):
		if key == 'PlayPause':
			...
			if key == 'Left':
				...
				bitmaptools.blit(self.bitmap,self.undo,0,0)
	...
	def paint(self):
		...
		if self.painting:
			bitmaptools.blit(self.undo,self.bitmap,0,0)
	...
	def loadSaveContents(self, save):
		...
		bitmaptools.blit(self.bitmap,self.nextframe,0,0)
		#self.undo.blit(0,0,self.nextframe)
		bitmaptools.blit(self.undo,self.nextframe,0,0)
		self.caretGroup.hidden = True
		self.lastload = time.monotonic()
		self.device.gc(1)
	...
	def saveCurrentToDisk(self, direction:int=0):
		...
		if self.currentSaveFilename == 'Unnamed':
			new = True
			self.currentSaveFilename = str(time.monotonic_ns())
		else:
			self.device.copyData('/paintsaves/'+self.currentSaveFilename+'.json', '/paintsaves_backup/'+str(time.monotonic_ns())+'.json')
			#print('copied', '/paintsaves/'+self.currentSaveFilename+'.json', 'to', '/paintsaves_backup/'+str(time.monotonic_ns())+'.json')
			new = False
