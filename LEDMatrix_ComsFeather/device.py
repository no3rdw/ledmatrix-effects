...
class Device:
	def __init__(self):
		...
		# IR receiver setup 
		self.ir_receiver = pulseio.PulseIn(board.A1, maxlen=120, idle_state=True)
	...
	def sendCode(self, code):
		if not self.buttonPause:
			...
			if len(foundAt):
				self.lastCodeSent = self.prepMessage(code)
				self.uart.write(self.lastCodeSent)
				self.neokey.pixels[0] = (255,0,0)
				self.neokey.pixels[1] = (255,0,0)
				self.neokey.pixels[2] = (255,0,0)
				self.neokey.pixels[3] = (255,0,0)
				#self.decoder._unparsed_pulses.clear()
				#self.decoder.pulses.clear()
				print("Sent:", self.lastCodeSent)	
		else:
	...
	def getWeather(self):
		...
		with self.requests.get(JSON_URL) as response:
			...
			return self.prepMessage(json.dumps(myresp))